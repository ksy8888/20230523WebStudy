...
<%
    ...
    List<FoodBean> list = dao.foodFindData(curpage, fd);	//(page,addr)
    //5. 총페이지 읽기 
  	...
  	<table class="table">
  		<tr>
  		 ...
  		 <form method="post" action="food_find.jsp" id="frm">
  		  <input type=text id="addr" name=addr size=20 class="input-sm" value="<%=fd%>">	<!-- 검색란에 "마포" 기본설정 -->
...
<!-- 페이지 -->    
    <div class="row">
    	...
    	<ul class="pagination">
    	<%
    		if(startPage>1) {	 //startPage >> 1 11 21... //1페이지의 이전은 없음
    	%>
    	...
    	%>
		 <%-- <li><a href="#"><%=i %></a></li> --%> <!--  <%-- <%=i %>: 123..페이지버튼 --%>  -->
		<%-- <li <%= i==curpage?"class=active":"" %>><a href="#"><%=i %></a></li> --%><!-- "class=active"현재페이지버튼 색칠 -->
