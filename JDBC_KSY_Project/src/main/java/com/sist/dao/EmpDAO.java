...
public class EmpDAO {
	...
	public List<Integer> empGetMgrData() {
		...
		//제네릭에는 반드시 클래스형만 사용 (제네릭스: 면접***) >> 데이터형의 표준화. 데이터형을 통으로 바꿔주는 역할
		try {
			conn = db.getConnection();
...
*/
			String sql = "SELECT empno, ename,job,TO_CHAR(hiredate,'YYYY-MM-DD'),TO_CHAR(sal,'L999,999'),NVL(comm,0),dname,loc,grade "
						...
						+ "WHERE me.deptno=md.deptno AND sal BETWEEN losal AND hisal";
			ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery();
			while(rs.next()) {
				EmpVO vo = new EmpVO();
...
*/			
			...
			// ?에 값을 채운다
			ps.setInt(1, empno);	//1번 물음표에 empno값 넣음
			ResultSet rs = ps.executeQuery();
			rs.next();
				vo.setEmpno(rs.getInt(1));
				vo.setEname(rs.getString(2));
	...
	//수정 데이터 읽기
	public EmpVO empUpdateData(int empno) {	//int empno >> primary key가 들어옴
		EmpVO vo = new EmpVO();
		try {
			conn = db.getConnection();
