...
<%
	...
	String dong = request.getParameter("dong");	//처음엔 값 들어오지 않았기 때문에 null > 
	int count=0;
	List<ZipcodeVO> list = null;	//값을 받아서 들어갈때 null값 처리를 해야함 _
	//count에 null이 들어감 >> 500오류 
	if(dong!=null) {	//값 입력이 되어야지만 연결되게 설정
		count=dao.postfindCount(dong);
		list = dao.postfind(dong);
	}
	// Model1(JSP) => MV구조 => MVC구조
...
<%--
	...
	1) 지시자
		...
		= include 지시자 : JSP안에 특정 위치 다른 JSP를 포함 
			<%@ include file="a.jsp"%>
	...
	3) JSP 액션 태그
		...
		<jsp:param> : 데이터 전송할때 
		<jsp: include> : 특정 위치에 다른 JSP를 추가할때
		<jsp: forward>: 화면 이동
			=sendRedirect() => request를 초기화
			=forward() => request를 계속 유지
	4) ****JSTL / EL
		=> 태그로 자바 라이브러리를 만들어준다 (JSTL)
		=> 화면 출력 => <% = %> (X) => ${}	   (EL)
...
let ok=(zip,addr)=> {
	//opener => js7.html (팝업창을 띄워준 위치) , js7파일의	window.open("post.jsp","post","width=500,height=300,scrollbars=yes");
	// 000-000
	opener.frm.post1.value=zip.substring(0,3);
	opener.frm.post2.value=zip.substring(4,7);
	opener.frm.addr.value=addr;
	self.close();	//자신 => post.jsp파일
}
</script>
</head>
<body>
	<div class="container">
		...
		<h1>우편번호 검색</h1>
		<div class="row">
		<form method=post action="post.jsp">	<!--String dong = request.getParameter("dong"); -->
		 <table class="table">
		  <tr>
		   <td>
