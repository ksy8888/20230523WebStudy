...
<%--
	...
	표현식)
		연산자
		  ...
		  비교 연산 ( ==(eq) , !=(ne) , <(lt) , >(gt) , <= , >= )
		  	${10 eq 10} => ${10==10}
		  	*** 날짜,문자열을 포함한다
		  		    ---- (한글 => 가나다라... , 알파벳 => ABC... abcd..) 
		  논리 연산 (&& (and) , || (or) )
		  		   && => 예약 가능 날짜
		  		   || => 예약이 불가능 날짜
		  상항 연산자 : 페이지 , select_ 체크박스 , radio...
		  			${sex=='남자'?"checked":""}
		  			${조건?"값":"값"}
	 ...
	 ----
	  		 ...
	  		 반복문
	  		 	<c:forEach> => for
	  		 		...
	  		 		=> <c:forEach var="i" begin="0" end="10" step="1">
	  		 			...
	  		 			*** 단점
	  		 			for(int i=10; i>=0; i--)
	  		 			-------------------------- <c:forEach>가 없다 => order by 사용
	  		 										=> i++만 가능						
	  		 	<c:forEach> => 향상된 for (for-each)
	  		 		  자바 for(BoardVO vo : list)
	  ...
	  fmt : 변환 (날짜변환 , 숫자변환 : DecimalFormat)
	    		-----	------			|
	    		SimpleDateFormat	<fmt: formatNumbe value="" pattern="999,999">
	    				|
 ...
 <%--
  ...
  <%
  		...
  		//EL은 사용할 수 없다 (EL ${} => request,session에 저장을 해야 사용 가능)
