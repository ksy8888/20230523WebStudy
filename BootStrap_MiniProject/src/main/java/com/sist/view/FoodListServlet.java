...
public class FoodListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<FoodVO1> clist = new ArrayList<FoodVO1>();
		if(cookies !=null) {
			for(int i = cookies.length-1; i>=0; i--) {		//최신을 먼저 읽어온다
				if(cookies[i].getName().startsWith("food_")) {
					//new Cookie("food_"+fno, fno)
					String fno = cookies[i].getValue();
					FoodVO1 vo = dao.foodDetailData(Integer.parseInt(fno));
		...
		if(clist != null && clist.size()>0) {
			for(FoodVO1 vo : clist) {
				if(i>7) break;
				out.println("<div class=\"col-md-3\">");	//한줄에 4개 출력 >> 12/3
				out.println("<div class=\"thumbnail\">");
				out.println("<a href=\"MainServlet?mode=3&fno="+vo.getFno()+"\">");	//카테고리의 번호에 해당하는
