...
public class FoodServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("let temp=$('#user-table > tbody > tr > td: nth-child(5n+2):contains(\"'+k+'\"')"); //5n:cols갯수 , +2:2번째위치 2열
		...
		out.print("<tr>");
		out.print("<td>");
		//input[type="text"] >> input type이 text 하나일때
		out.print("<input type = text size=30 id=keyword>");
		out.print("</td>");
		out.print("</tr>");
		out.println("</table>");
		...
		out.println("</tr>");
		out.println("</thead>");
		out.println("<tbody>");	//실제값을 출력하는 부분 Vue,React(View)=> table출력이 안됨
		for(FoodVO vo:list) {
			out.println("<tr>");
