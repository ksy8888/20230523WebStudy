...
<body>
	...
	<h2 class="sectiontitle">${sessionScope.name }님의 맛집 예약 목록</h2>
	<table class="table">
	 <tr>
	 ...
	 <c:forEach var="vo" items="${list }">
	  ...
	  <td class="text-center">
	     <c:if test="${vo.rok == 'y' }">
	       <span class="btn btn-sm btn-success checks" data-fno="${vo.fno }" data-no="${vo.no }">예약 완료</span>
	     </c:if>
	     <c:if test="${vo.rok == 'n' }">
	       <span class="btn btn-sm btn-default">예약 대기</span>
	     </c:if>
	  </td>
	  </tr>
	 </c:forEach> 
