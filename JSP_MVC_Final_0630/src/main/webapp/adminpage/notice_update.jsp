...
<%--
    ...
    Model에서 처리 (비지니스 로직) : Model (Model,VO,DAO) 
     		Model: 브라우저 연결
     		VO : 데이터를 모아서 전송할 목적
     		DAO: 데이터베이스만 연결
    JSP에서 화면애 데이터 출력 (프레젠테이션로직)
...
<body>
   ...
   <table class="table">
    <tr>
     ...
     <td width=80%>
       <select name="type" class="input-sm">
        ...
        <option value="2" ${vo.type==2?"selected":"" }>이벤트 공지</option>
        <option value="3" ${vo.type==3?"selected":"" }>맛집 공지</option>
        <option value="4" ${vo.type==4?"selected":"" }>여행 공지</option>
        <option value="5" ${vo.type==5?"selected":"" }>레시피 공지</option>
       </select>
     </td>
    </tr> 
    <tr>
    ...
    <tr>
     ...
     <td width=80%>
       <textarea rows="10" cols="50" name="content">${vo.content }(수정본)</textarea>
     </td>
    </tr> 
    <tr>
