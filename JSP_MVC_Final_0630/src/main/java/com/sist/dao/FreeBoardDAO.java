...
public class FreeBoardDAO {
	...
	// 기능
	// 1. 목록 출력 => 인라인뷰_select안에 select => (페이징)			[1]
	public List<FreeBoardVO> freeboardListData(int page) {
		List<FreeBoardVO> list = new ArrayList<FreeBoardVO>();
		
		try {
			conn = db.getConnection();
			String sql = "SELECT no,name,subject,TO_CHAR(regdate,'yyyy-mm-dd'),hit,rownum "
					+ "FROM (SELECT no,name,subject,regdate,hit, rownum as num "
					+ "FROM (SELECT /*+ INDEX_DESC(project_freeboard pf_no_pk)*/no,name,subject,regdate,hit "
	...
	// 2. Top7 => rownum (HIT가 많은 위에서 7개) => Top-N
	// 3. 글쓰기		[2]
