...
<body>
	<div class="container">
	 <h1>수정하기</h1>
	 <div class="row">
	 <form method=post action="update_ok.jsp" name=frm>  <!-- insert_ok는 DB랑 연동만 >> 폼입력값 넘겨줌 -->
	  			<%-- multipart/form-data : fileuplead 프로토콜 => POST --%>
	  <table class="table">
	   <tr>
	   	<th class="text-center danger" width=15%>이름</th>
	   	<td width=85%>
	   		<input type=text name=name class="input-sm" size=15 value="<%=vo.getName()%>">
	   		<input type=hidden name=no value="<%=no %>">	<!-- 수정no -->
	   	</td>
	   </tr>
	   <tr>
