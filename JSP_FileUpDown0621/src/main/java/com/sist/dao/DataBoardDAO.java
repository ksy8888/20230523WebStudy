...
public class DataBoardDAO {
	// 연결 객체 => Socket
		...
		public List<DataBoardVO> databoardListData(int page) {
			...
			try {
				getConnection();
				String sql="SELECT no,subject,name,TO_CHAR(regdate,'YYYY-MM-DD'),hit,num " //rownum을 컬럼으로
							+"FROM (SELECT no,subject,name,regdate,hit,rownum as num "	//insert된 순서대로 들어옴
							+"FROM (SELECT /*+ INDEX_DESC(jspDataBoard jd_no_pk)*/ no,subject,name,regdate,hit " //rownum 결정해서 들어옴
							+"FROM jspDataBoard)) " //57줄~59까지 하나의 테이블로 생성
		...
		public DataBoardVO databoardDetailData(int no, int type) {
			...
			try {
				getConnection();
				if(type==0) {	//0일때만 조회수 증가 >> update,delete때는 조회수 증가 안하게 처리
...
////////////////////////////////////////////댓글
		...
		//2.댓글 읽기 (추가된 내용 읽어오기)
		public List<ReplyVO> replyListData(int bno) {	//bno의 게시글에 해당하는 댓글 불러옴
		...
		public void replyUpdate(int no, String msg) {
			try {
				getConnection();
				String sql = "UPDATE jspReply SET msg=? "
							+"WHERE no=?";
				ps = conn.prepareStatement(sql);
				ps.setString(1,  msg);
				ps.setInt(2, no);
