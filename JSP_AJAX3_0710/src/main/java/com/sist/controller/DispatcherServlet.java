 ...
 *  	=> View
 *  구현단(비즈니스 로직) : Java => 데이터베이스 연결 (요청 처리)
 *  	=> Model (VO,DAO,Model,Manager)
 *  연결 (화면,구현) : 서블릿
 *  	=> Controller
 ----------------------------------------------------
  	model 1 (순수하게 JSP) => 장점: 분석하기 편리, 단점: 확장성, 추가, 재사용이 어렵다
  	model 2 (JSP/java) => 단점: 복잡함 , 장점: 여러명이 동시에 개발, 추가,확장성,재사용 용이 => 유지보수
  					   => Controller에 집중이 된다
  	...
  	* Controller를 찾는 방법 : .do (URL주소 설정)
  	...
  	사용자 요청 ---------------- Controller(DispatcherServlet) ----------- Model ------ DAO
  							 (Front Controller)
  							    ...
  							    => 요청처리에 대한 메소드를 찾아서 호출 
  							    => 결과값을 request에 담아서 JSP로 전송
  							    => Spring / Structs (FilterDispatcher)
  							       ----------------
  							          |
  							       DispatcherServlet
...
public class DispatcherServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//1. 사용자가 보내준 URI주소를 읽어온다
		String cmd = request.getRequestURI();
		System.out.println("cmd="+cmd);
		//http://localhost/JSP_AJAX3_0710/diary3/diary3.do
		//				   ---------------------------------URI
		...
		try {
			...
			for(String cls:clsList) {
				...
				Method[] methods = clsName.getDeclaredMethods(); //메소드 전체를 읽어온다
				//메소드 위에 있는 어노테이션 읽기
