...
<jsp:useBean id="dao" class="com.sist.goodsdao.GoodsDAO"/>

<%
	//사용자가 보내준 상품번호를 받는ㄷ
	String no = request.getParameter("no");	//goods.jsp 72줄
	//goods_detail.jsp?no=1
	
	GoodsBean vo  = dao.goodsDetailData(Integer.parseInt(no));
	String s = vo.getPrice();
	s=s.replaceAll("[^0-9]","");	//원 , 지우기 ->숫자를 제외한 나머지 공백으로
			...
			<td width=35% class="text-center" rowspan="6">		  
			 ...
			 <tr>
			 	...
			 	<%
			 		...
			 		if(id!=null) {			 	
			 	%>
			 		<input type="submit" value="장바구니" class="btn btn-sm btn-success" id="cartBtn">
			 	<%
			 		}
			 	%>	
			 	</td>
