...
<%--
	...
	클래스명 	(내장객체)			(필요시마다 생성)
			...
			------------------------------------------------------------
	저장값	자바에서 사용되는 모든 클래스 문자열만 저장이 가능 >> cookie
			(Object)		
	...
	Cookie
		1) Cookie 생성
			Cookie cookie = new Cookie(String key, String value)
							------------------------------------
		2) 저장 기간 설정
			cookie.setMaxAge(초단위로 저장)	=> 0이면 삭제 cookie.setMaxAge(0)
				   -------------------
		3) 저장 path 지정
			cookie.setPath("/")
				   ------------
	...
	=> 서버에 저장 (보안이 뛰어나다)
		...
		***5) 세션 전체 삭제
		...
		저장방식 => Map방식(key,value)
				value는 중복이 가능
				key는 중복이 불가능  ===> 덮어쓰기(key는 마지막값으로 덮어씌워짐 이전값 삭제)
